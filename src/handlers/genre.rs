...
pub async fn get_genre_by_id_handler(State(state): State<AppState>, Path(id): Path<i64>) -> Result<Json<GenreDto>, BooksError> {
...
#[axum::debug_handler]
pub async fn get_genres_handler(State(state): State<AppState>,) -> Result<Json<Vec<GenreDto>>, BooksError> {
...
#[axum::debug_handler]
pub async fn create_genre_handler(State(state): State<AppState>, Json(genre_dto): Json<CreateGenreDto>) -> Result<Json<GenreDto>, BooksError>{
